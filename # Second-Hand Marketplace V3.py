...
#现在这个版本增加了搜索功能，用户可以输入物品名称进行搜索，并显示匹配的结果。用户还可以选择购买搜索结果中的物品，并计算平台手续费。

#之后还以该增加选择卖出商品时可以自行设置卖出价格，现在的系统会根据成色推荐一个价格，并提示用户是否接受推荐价格。
# 但是如果用户不接受推荐的价格，还可以自行输入一个卖出价格（功能）。
